...
public class SudokuSolver {
	...
	private static String formatPuzzle(Puzzle p) {
		...
		final String rowSectorSeparator = "|---+---+---|\n";

		String[] formatedRows = CellFunctions.rangeStream().map(formatRow).collect(Collectors.toList())
				.toArray(new String[9]);

		return rowSectorSeparator + formatedRows[0] + formatedRows[1] + formatedRows[2] //
				+ rowSectorSeparator + formatedRows[3] + formatedRows[4] + formatedRows[5] //
				+ rowSectorSeparator + formatedRows[6] + formatedRows[7] + formatedRows[8] //
				+ rowSectorSeparator;
	}
